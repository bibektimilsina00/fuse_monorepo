    ...
    >
      ...
      <form onSubmit={handleSubmit} className="flex flex-col gap-4">
        ...
        <div className="flex flex-col gap-1.5 p-2.5 bg-bg/50 border border-border-faint rounded-[8px] mt-1 text-[11px]">
          <div className="flex items-center gap-1.5 text-text-mute font-medium mb-0.5">
            Password requirements:
          </div>
          <div className="flex items-center gap-2">
            <span
              className={`flex items-center justify-center w-3.5 h-3.5 rounded-full ${reqs.length ? "bg-ok/10 text-ok" : "bg-text-faint/10 text-text-faint"}`}
            >
              {reqs.length ? (
            ...
            <span className={reqs.length ? "text-text" : "text-text-faint"}>
              At least 8 characters
            </span>
          </div>
          <div className="flex items-center gap-2">
            <span
              className={`flex items-center justify-center w-3.5 h-3.5 rounded-full ${reqs.numberOrSymbol ? "bg-ok/10 text-ok" : "bg-text-faint/10 text-text-faint"}`}
            >
              {reqs.numberOrSymbol ? (
                <Check size={10} strokeWidth={3} />
              ) : (
            ...
            <span
              className={reqs.numberOrSymbol ? "text-text" : "text-text-faint"}
            >
          ...
          <div className="flex items-center gap-2">
            <span
              className={`flex items-center justify-center w-3.5 h-3.5 rounded-full ${reqs.match ? "bg-ok/10 text-ok" : "bg-text-faint/10 text-text-faint"}`}
            >
              {reqs.match ? (
                <Check size={10} strokeWidth={3} />
              ) : (
            ...
            <span className={reqs.match ? "text-text" : "text-text-faint"}>
              Passwords match
            </span>
          </div>
        </div>
      ...
      <div className="mt-6 pt-4 border-t border-border-faint text-center text-[12px] text-text-mute">
        ...
        <Link
          to={
            rawRedirect
              ? `${APP_ROUTES.LOGIN}?redirect=${encodeURIComponent(rawRedirect)}`
              : APP_ROUTES.LOGIN
          }

...
import {
  ...
  type GraphPatch,
} from "../stores/workflowEditorStore";
...
export function useWorkflowCollaboration(workflowId: string | null) {
  ...
  const applyRemoteGraphPatch = useWorkflowEditorStore(
    (s) => s.applyRemoteGraphPatch,
  );
  ...
  useEffect(() => {
    ...
    ws.onerror = (event) =>
      logger.error("[Collaboration] WebSocket error", event);
    ws.onmessage = (event) => {
      ...
      if (message.type === "session.ready" && message.session) {
        ...
        setMyUserId(message.session.user_id);
        return;
      }
      if (message.type === "presence.snapshot") {
        message.sessions?.forEach((session) =>
          setPresence(toPresence(session)),
        );
        return;
      }
      if (message.type === "presence.joined" && message.session)
        setPresence(toPresence(message.session));
      if (message.type === "presence.left" && message.session)
        removePresence(message.session.session_id);
      ...
      if (message.type === "graph.saved") {
        ...
        if (typeof version === "number")
          useWorkflowEditorStore.getState().markSaved(version);
      }
    };
  ...
  const send = useCallback(
    (type: string, payload: Record<string, unknown>, patchId?: string) => {
  ...
  const sendGraphSaved = useCallback(
    (version: number) => {
...
function isGraphPatch(value: unknown): value is GraphPatch {
  ...
  return (
    patch.operation === "node.position" || patch.operation === "graph.replace"
  );
}

...
export function InviteForm() {
  ...
  const [latestInvite, setLatestInvite] = useState<WorkspaceInvite | null>(
    null,
  );
  ...
  return (
    <div className="flex flex-col gap-[10px]">
      <form
        className="grid grid-cols-[1fr_120px_auto] gap-[8px]"
      ...
      {latestInvite && (
        <div className="flex items-center gap-[10px] px-[14px] py-[10px] bg-[var(--surface)] border border-[var(--border-faint)] rounded-[9px] mt-2">
          ...
          />
          <span className="flex-1 min-w-0 text-[11.5px] font-mono text-[var(--text-mute)] overflow-hidden text-ellipsis whitespace-nowrap">
            {latestInvite.invite_url}
          </span>
          <button
            className="inline-flex items-center gap-[5px] px-[10px] py-[5px] rounded-[7px] bg-[var(--surface-2)] border border-[var(--border-faint)] text-[var(--text-mute)] text-[11.5px] font-medium cursor-pointer transition-colors duration-80 whitespace-nowrap hover:bg-[var(--surface-3)] hover:text-[var(--text)]"
            onClick={handleCopy}
            type="button"
          >
            {copied ? (
      ...
      {createInvite.isError && (
        <p className="text-[12px] text-[var(--err)] m-0">

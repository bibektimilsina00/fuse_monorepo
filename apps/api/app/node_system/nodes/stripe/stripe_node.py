...
class StripeNode(BaseNode[StripeProperties]):
        ...
        return NodeMetadata(
            ...
            properties=[
                ...
                },
                {
                    ...
                    "description": "Amount in smallest currency unit (e.g. 1000 = $10.00)",
                },
                {
                    "name": "currency",
                    "label": "Currency",
                ...
                },
                {
                    ...
                    "condition": {
                        "field": "operation",
                        "value": ["get_payment_intent", "create_refund"],
                    },
                },
                {
                    "name": "customer_id",
                    ...
                    "condition": {
                        "field": "operation",
                        "value": ["get_customer", "create_payment_intent"],
                    },
                },
                {
                    "name": "email",
                ...
                },
                {
                    ...
                    "condition": {
                        "field": "operation",
                        "value": ["create_payment_intent", "create_customer"],
                    },
                },
                {
                    "name": "limit",
                    ...
                    "condition": {
                        "field": "operation",
                        "value": ["list_payments", "list_customers", "list_invoices"],
                    },
                },
            ],
            inputs=1,
    ...
    async def execute(self, input_data: dict[str, Any], context: NodeContext) -> NodeResult:
        if not self._api_key():
            return NodeResult(success=False, error="Stripe secret key required.")
        op = self.props.operation
        try:
            async with httpx.AsyncClient(timeout=30) as client:
                if op == "create_payment_intent":
                    if not self.props.amount:
                        return NodeResult(success=False, error="Amount required.")
                    data: dict = {"amount": self.props.amount, "currency": self.props.currency}
                    if self.props.customer_id:
                        data["customer"] = self.props.customer_id
                    if self.props.description:
                        data["description"] = self.props.description
                    r = await client.post(
                        f"{STRIPE_API}/payment_intents", auth=self._auth(), data=data
                    )
                    r.raise_for_status()
                    return NodeResult(success=True, output_data=r.json())

                elif op == "get_payment_intent":
                    if not self.props.payment_intent_id:
                        return NodeResult(success=False, error="Payment Intent ID required.")
                    r = await client.get(
                        f"{STRIPE_API}/payment_intents/{self.props.payment_intent_id}",
                        auth=self._auth(),
                    )
                    r.raise_for_status()
                    return NodeResult(success=True, output_data=r.json())
                    ...
                    return NodeResult(
                        success=True,
                        output_data={"data": d.get("data", []), "count": len(d.get("data", []))},
                    )

                elif op == "create_customer":
                    data = {}
                    if self.props.email:
                        data["email"] = self.props.email
                    if self.props.name:
                    ...
                    if not self.props.customer_id:
                        return NodeResult(success=False, error="Customer ID required.")
                    r = await client.get(
                        f"{STRIPE_API}/customers/{self.props.customer_id}", auth=self._auth()
                    )
                    r.raise_for_status()
                    return NodeResult(success=True, output_data=r.json())
                    ...
                    r = await client.post(
                        ...
                        auth=self._auth(),
                        data={"payment_intent": self.props.payment_intent_id},
                    )
                    r.raise_for_status()
                    return NodeResult(success=True, output_data=r.json())
                    ...
                    if self.props.customer_id:
                        params["customer"] = self.props.customer_id
            ...
            return NodeResult(
                success=False, error=f"Stripe API {e.response.status_code}: {e.response.text[:200]}"
            )

        ...
        return NodeMetadata(
            ...
            properties=[
                ...
                },
                {
                    ...
                    "condition": {
                        "field": "operation",
                        "value": ["send_message", "send_embed", "get_messages", "delete_message"],
                    },
                },
                {
                    "name": "content",
                ...
                },
                {
                    ...
                    "label": "Embed (JSON)",
                    "type": "json",
                    "condition": {"field": "operation", "value": "send_embed"},
                },
                {
                    "name": "message_id",
                    "label": "Message ID",
                    "type": "string",
                    "condition": {"field": "operation", "value": "delete_message"},
                },
                {
                    "name": "guild_id",
                    "label": "Guild/Server ID",
                    "type": "string",
                    "condition": {"field": "operation", "value": "get_guild"},
                },
                {
                    "name": "user_id",
                    "label": "User ID",
    ...
    async def execute(self, input_data: dict[str, Any], context: NodeContext) -> NodeResult:
        ...
        try:
            async with httpx.AsyncClient(timeout=30) as client:
                if op == "send_message":
                    if not self.props.channel_id:
                        return NodeResult(success=False, error="channel_id required")
                    r = await client.post(
                        f"{DISCORD_API}/channels/{self.props.channel_id}/messages",
                        headers=headers,
                        json={"content": self.props.content or ""},
                    )
                    r.raise_for_status()
                    ...
                    payload: dict = {}
                    if self.props.embed:
                        payload["embeds"] = [self.props.embed]
                    r = await client.post(
                        f"{DISCORD_API}/channels/{self.props.channel_id}/messages",
                        headers=headers,
                        json=payload,
                    ...
                    r.raise_for_status()
                    return NodeResult(
                        success=True, output_data={"messages": r.json(), "count": len(r.json())}
                    )
                elif op == "delete_message":
                    if not self.props.channel_id or not self.props.message_id:
                        return NodeResult(success=False, error="channel_id and message_id required")
                    r = await client.delete(
                        f"{DISCORD_API}/channels/{self.props.channel_id}/messages/{self.props.message_id}",
                        headers=headers,
                    )
                    r.raise_for_status()
                    return NodeResult(success=True, output_data={"deleted": True})
                elif op == "get_guild":
                    if not self.props.guild_id:
                        return NodeResult(success=False, error="guild_id required")
                    r = await client.get(
                        f"{DISCORD_API}/guilds/{self.props.guild_id}", headers=headers
                    )
                    r.raise_for_status()
                    return NodeResult(success=True, output_data=r.json())
                elif op == "get_user":
            ...
            return NodeResult(
                success=False,
                error=f"Discord API error {e.response.status_code}: {e.response.text[:200]}",
            )
